...
#' @param bottom_align_column_labels Text in column labels and spanners is
...
bhhi_format_table <- function(table,
  ...
  if (bottom_align_column_labels) {
    table <- gt::tab_style(
      table,
      gt::cell_text(v_align = "bottom"),
      list(gt::cells_column_spanners(), gt::cells_column_labels())
    )
  }
